...
//idially the const_time operations can be replaced with more efficient ones, but I haven't checked if the crypto library offers unsafe alternatives
...
int main(int argc, char *argv[]){
	...
	//FILE* pas_fd = fopen("passwords.txt", "r");
	//FILE* out_fd = fopen("output.csv", "w");
	...
	if (pas_fd != NULL) {
		...
		while (fgets(line, sizeof(line), pas_fd)) {
		    ...
		    for(j=0;j<address_num;j++){//2, 1, 2, 1, 1, 1, 1, 2, 3, 1, 2, 3, 1, 1, 1, 1, 9, 1, 1, 2 for all default
				addr2[5]=i;
				iters = sae_derive_pwe_ffc_test(mysae,addr1,addr2,(u8 *) line, os_strlen(line));
			      ...
			      const char *identifier)
{
	...
	int default_groups[] = { 19, 24, 29, 0 };//shmuel segal
	int i;
...
u16 sae_group_allowed(struct sae_data *sae, int *allowed_groups, u16 group)
{
	...
	if (sae->state == SAE_COMMITTED && group != sae->group) {
		...
		//return WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED; shmuel segal
	}
